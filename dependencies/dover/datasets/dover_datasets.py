...
def spatial_temporal_view_decomposition(
    ...
    if video_path.endswith(".yuv"):
        ...
        decord.bridge.set_bridge("torch")
        vreader = VideoReader(video_path, num_threads = 1, ctx=cpu(0), fault_tol = 2) # Add additional arguments to prevent hangs
        ...
        # Change method to accomodate issues with original video
        # If for whatever reason frame seeking fails, just abort this entire video
        frame_dict = {}
        for i in np.unique(all_frame_inds):
            try:
                frame_dict[i] = vreader[i]
            except Exception:
                frame_dict = {}
                break
