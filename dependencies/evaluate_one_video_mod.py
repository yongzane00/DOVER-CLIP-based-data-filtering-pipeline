## Essentially the same as evaluate_one_video, but with the file paths modified
...
def rescale_results(results: list, vname="undefined"):
    ...
    for abbr, full_name in dbs.items():
        with open(f"./DOVER/dover_predictions/val-{abbr}.pkl", "rb") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="./DOVER/dover-mobile.yml", help="the option file"
    )
    ...
    parser.add_argument(
        "-d", "--device", type=str, default="cpu", help="the running device" # Imagine having an NVDIA gpu smoge
    )
